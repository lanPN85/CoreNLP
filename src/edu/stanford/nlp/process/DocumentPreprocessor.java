...
public class DocumentPreprocessor implements Iterable<List<HasWord>> {
    ...
    public enum DocType {Plain, XML, StdIn}
    ...
    private class StdInIterator implements Iterator<List<HasWord>> {
        ...
        StdInIterator() {
            if (tokenizerFactory == null) {
                tokenizer = WhitespaceTokenizer.
                        newCoreLabelWhitespaceTokenizer(inputReader, true);
            } else {
        ...
        public boolean hasNext() {
            try {
                next = ((BufferedReader)inputReader).readLine();
                return next != null && !next.trim().equals("");
            } catch (IOException e) {
                return false;
            }
        }
